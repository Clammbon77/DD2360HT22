...
void BFSGraph( int argc, char** argv) 
{
  ...
  cudaMemPrefetchAsync(graph_nodes, sizeof(Node) * no_of_nodes, devID);
	cudaMemPrefetchAsync(updating_graph_mask, sizeof(bool) * no_of_nodes, devID);
	cudaMemPrefetchAsync(u_cost, sizeof(int) * no_of_nodes, devID);
  cudaMemPrefetchAsync(graph_mask, sizeof(bool) * no_of_nodes, devID);
	cudaMemPrefetchAsync(graph_visited, sizeof(bool) * no_of_nodes, devID);

  //ALLOCATE the Edge List UM 
	int* graph_edges;
	...
	cudaMemPrefetchAsync(graph_edges, sizeof(int) * edge_list_size, devID);
	...
	do
	{
		...
		stop=false;
		cudaMemcpy(over, &stop, sizeof(bool),cudaMemcpyHostToHost);
		...
		cudaMemcpy(&stop, over, sizeof(bool),cudaMemcpyHostToHost); 

		k++;
	}
	while(stop);

  cudaMemPrefetchAsync(u_cost, sizeof(int)*no_of_nodes, cudaCpuDeviceId);
